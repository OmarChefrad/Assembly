section .data
    ...
    menu db "1. View Rooms", 10, "2. Book a Room", 10, "3. Cancel Booking", 10, "4. View Booked Rooms", 10, "5. Exit", 0
    prompt db "Choose an option: ", 0
    book_prompt db "Enter room number to book: ", 0
    cancel_prompt db "Enter room number to cancel: ", 0
    ...
    cancel_success db "Booking cancelled successfully!", 10, 0
    already_booked db "Room already booked!", 10, 0
    not_booked db "Room not booked yet!", 10, 0
...
cancel_booking:
    ...
    movzx ebx, byte [booked_rooms + eax]
    cmp ebx, 0
    je not_booked_message

    ; Cancel the booking and clear the customer name
    mov byte [booked_rooms + eax], 0
    mov esi, eax
    lea edi, [customer_names + esi * 10]
    mov ecx, 10
    xor al, al
    rep stosb

    ; Display cancel success message
    mov eax, 4
    mov ebx, 1
    mov ecx, cancel_success
    mov edx, 28
    int 0x80
    jmp _start

not_booked_message:
    ; Display not booked message
    mov eax, 4
    mov ebx, 1
    mov ecx, not_booked
    mov edx, 18
